...
class WebUI_API:
    ...
    def get_vram(host, port, *args):
        res = WebUI_API._utils_requests(host, port, "/sdapi/v1/memory", {}, True, False)
        ...
        return vram_free, vram_used, vram_total, vram_percent
    ...
    def get_or_match_vae(host, port, *args):
        ...
        vaes = ["None", "Automatic"] + [vae["model_name"] for vae in response]

        if not args:
            return vaes
        else:
            user_vae = args[0]
            return TextOptimization.find_similar_str(vaes, user_vae)
    ...
    def get_or_match_hires_upscaler(host, port, *args):
        return list(map(lambda x: x.value, WebUI_API.HiResUpscaler))

    @staticmethod
    def get_or_match_extra_upscaler(host, port, *args):
        return list(map(lambda x: x.value, WebUI_API.ExtraUpscaler))

    @staticmethod
    def get_or_match_loras(host, port, *args):
        WebUI_API._utils_requests(host, port, "/sdapi/v1/refresh-loras", {})

        response: list = WebUI_API._utils_requests(
            host,
            port,
            "/sdapi/v1/loras",
            js={},
        ...
        loras = [lora["name"] for lora in response]

        if not args:
            return loras
        else:
            user_lora = args[0]
            return TextOptimization.find_similar_str(loras, user_lora)

    @staticmethod
    def get_or_match_embeddings(host, port, *args):
        response: dict = WebUI_API._utils_requests(
            host,
            port,
            "/sdapi/v1/embeddings",
        ...
        for loaded in response["loaded"]:
            embeddings.append(str(loaded))

        if not args:
            return embeddings
        else:
            user_embeddings = args[0]
            return TextOptimization.find_similar_str(embeddings, user_embeddings)
